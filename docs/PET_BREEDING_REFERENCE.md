...
| `pet_breeding_mutation_decay_rate` | double | 0.0 | - | Divides the base chance by `1 + decay x (stat mutations the baby inherited)`. Potency is not counted. 0 = flat. There is no guard against negative values. | `PetDevice_Breeding.cs:180, 434` | Set 1.0 with `pet_trace` on: `breed.roll` chance falls. |
