...
namespace ACE.Server.Command.Handlers
{
    public static class AdminCommands
    {
        ...
        [CommandHandler("freeze", AccessLevel.Sentinel, CommandHandlerFlag.RequiresWorld, 0,
            "Freezes the selected target for 10 minutes or until unfrozen.",
            "[player name]\nFreezes the target player, preventing all movement, actions, and teleportation.")]
        public static void HandleFreeze(Session session, params string[] parameters)
        {
            ...
            if (parameters.Length > 0)
            {
                ...
                targetPlayer = PlayerManager.GetOnlinePlayer(playerName);

                if (targetPlayer == null)
                {
                    ChatPacket.SendServerMessage(session, $"Unable to find a player named {playerName}.", ChatMessageType.Broadcast);
                    return;
                }
            }
            else
            {
                ...
                if (objectId == ObjectGuid.Invalid || !objectId.IsPlayer())
                {
                    ChatPacket.SendServerMessage(session, "Please select a player target or specify a player name.", ChatMessageType.Broadcast);
                    return;
                }

                targetPlayer = PlayerManager.GetOnlinePlayer(objectId);

                if (targetPlayer == null)
                {
                    ChatPacket.SendServerMessage(session, "Selected target is not a valid player.", ChatMessageType.Broadcast);
                    return;
                }
            }

            var msg = "";
            if (PlayerManager.FreezePlayer(session.Player, targetPlayer.Name, false))
            {
                msg = $"{targetPlayer.Name} has been frozen for ten minutes.";
                targetPlayer.Session.Network.EnqueueSend(new GameMessageSystemChat($"You have been frozen by {session.Player.Name} for ten minutes.", ChatMessageType.Broadcast));
            }
            else
            {
                msg = $"Unable to freeze {targetPlayer.Name}.";
            }
        ...
        // permafreeze
        [CommandHandler("permafreeze", AccessLevel.Sentinel, CommandHandlerFlag.RequiresWorld, 0,
            "Permanently freezes the selected target until unfrozen by an admin.",
            "[player name]\nFreezes the target player indefinitely, preventing all movement, actions, and teleportation.")]
        public static void HandlePermaFreeze(Session session, params string[] parameters)
        {
            // @permafreeze - Permanently freezes the selected target until unfrozen.
            ...
            var msg = "";
            if (PlayerManager.FreezePlayer(session.Player, targetPlayer.Name, true))
            {
                msg = $"{targetPlayer.Name} has been permanently frozen.";
                targetPlayer.Session.Network.EnqueueSend(new GameMessageSystemChat($"You have been permanently frozen by {session.Player.Name}.", ChatMessageType.Broadcast));
            }
            else
            {
                msg = $"Unable to freeze {targetPlayer.Name}.";
        ...
        [CommandHandler("unfreeze", AccessLevel.Sentinel, CommandHandlerFlag.RequiresWorld, 0,
            "Unfreezes the selected target.",
            "[player name]\nRemoves freeze status from the target player.")]
        public static void HandleUnFreeze(Session session, params string[] parameters)
        {
            ...
            var msg = "";
            if (PlayerManager.UnfreezePlayer(session.Player, targetPlayer.Name))
            {
                msg = $"{targetPlayer.Name} has been unfrozen.";
                targetPlayer.Session.Network.EnqueueSend(new GameMessageSystemChat($"You have been unfrozen by {session.Player.Name}.", ChatMessageType.Broadcast));
            }
            else
            {
                msg = $"Unable to unfreeze {targetPlayer.Name}.";
            }
        ...
        [CommandHandler("limbo", AccessLevel.Sentinel, CommandHandlerFlag.RequiresWorld, 0,
            "Puts the selected target in limbo.",
            "[on|off] [player name]\nIn limbo: cannot deal/receive damage, no tells/channels, cannot salvage. Lasts 15 minutes.")]
        public static void HandleLimbo(Session session, params string[] parameters)
        {
            ...
            if (parameters.Length > 0)
            {
                ...
                else if (parameters[0].ToLower() == "on")
                {
                    enable = true;
                    if (parameters.Length > 1)
                        playerName = string.Join(" ", parameters.Skip(1));
                ...
                {
                    // Assume entire parameter is player name
            ...
            var msg = "";
            if (PlayerManager.SetPlayerLimbo(session.Player, targetPlayer.Name, enable))
            {
                if (enable)
                {
                    msg = $"{targetPlayer.Name} has been put in limbo for fifteen minutes.";
                    targetPlayer.Session.Network.EnqueueSend(new GameMessageSystemChat($"You have been put in limbo by {session.Player.Name} for fifteen minutes. You cannot deal or receive damage, receive tells or channel messages, or salvage items.", ChatMessageType.Broadcast));
                }
                else
                {
                    msg = $"{targetPlayer.Name} has been removed from limbo.";
                    targetPlayer.Session.Network.EnqueueSend(new GameMessageSystemChat($"You have been removed from limbo by {session.Player.Name}.", ChatMessageType.Broadcast));
                }
            }
            else
            {
                msg = $"Unable to modify limbo status for {targetPlayer.Name}.";
            }
