...
namespace ACE.Server.WorldObjects
{
    partial class Creature
    {
        ...
        public DeathMessage GetDeathMessage(DamageHistoryInfo lastDamagerInfo, DamageType damageType, bool criticalHit = false)
        {
            ...
            // Check if this was a split arrow kill and suppress death messages
            if (lastDamagerInfo.IsPlayer && lastDamager is Player)
            {
                // Check if the last split arrow projectile is still active and caused this death
                var lastSplitArrowGuid = GetProperty(PropertyInstanceId.LastSplitArrowProjectile);
                if (lastSplitArrowGuid.HasValue)
                {
                    ...
                    if (lastSplitArrow != null && lastSplitArrow.GetProperty(PropertyBool.IsSplitArrowKill) == true)
                    {
                        ...
                        if (splitArrowDeathMessage != null)
                        {
                            ...
                            {
                                playerKiller.Session.Network.EnqueueSend(new GameEventKillerNotification(playerKiller.Session, splitMsg));
                            }
                        }
