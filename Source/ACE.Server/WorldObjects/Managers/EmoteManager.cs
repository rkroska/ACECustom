                ...
                case EmoteType.GrantRandomQuestStamp:
                    ...
                    if (player != null)
                    {
                        try
                        {
                            ...
                            var randomIndex = ThreadSafeRandom.Next(0, eligibleStamps.Count - 1);
        ...
        private HashSet<string> GetPlayerQuestStamps(Player player, List<string> stampsToCheck)
        {
            ...
            try
            {
                using (var context = new AuthDbContext())
                {
                    ...
                    // Normalize case for case-insensitive matching (consistent with StringComparer.OrdinalIgnoreCase used in calling code)
                    var stampsToCheckLower = stampsToCheck.Select(s => s.ToLowerInvariant()).ToList();
                    var playerStamps = context.AccountQuest
                        .Where(x => x.AccountId == player.Account.AccountId 
                                 && x.NumTimesCompleted >= 1 
                                 && stampsToCheckLower.Contains(x.Quest.ToLower()))
                        .Select(x => x.Quest)
                        .ToList();
