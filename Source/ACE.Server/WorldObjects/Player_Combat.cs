    ...
    partial class Player
    {
        ...
        public DamageEvent DamageTarget(Creature target, WorldObject damageSource)
        {
            ...
            if (damageEvent.HasDamage)
            {
                ...
                {
                    // ILT: Monster Mana Barrier — pre-absorb before damage lands
                    if (target.GetProperty(ACE.Entity.Enum.Properties.PropertyBool.HasManaBarrier) == true)
                    {
            ...
            if (damageEvent.HasDamage && target.IsAlive)
            {
                ...
                if (!SquelchManager.Squelches.Contains(this, ChatMessageType.CombatSelf))
                {
                    if (damageEvent.AmountAbsorbed > 0)
                    {
                        ...
                        {
                            // Monster fully absorbed — player TakeDamage doesn't send this message
                        ...
                        // else: player target fully absorbed — already handled in Player.TakeDamage
                    }
                    else
                    {
        ...
        public override void TakeDamageOverTime(float _amount, DamageType damageType)
        {
            ...
            // ── Mana Barrier ───────────────────────────────────────────────────────
            var mbResult = TryAbsorbWithManaBarrier(ref _amount, damageType);
            if (mbResult.FullyAbsorbed)
            {
                var mbPtWord = mbResult.AmountAbsorbed == 1 ? "point" : "points";
                SendMessage($"Mana Barrier absorbed {mbResult.AmountAbsorbed} {mbPtWord} of periodic damage!", ChatMessageType.Magic);
                return;
            }
            // ─────────────────────────────────────────────────────────────────────
        ...
        private int TakeDamageInternal(WorldObject source, DamageType damageType, float _amount, BodyPart bodyPart, out uint amountAbsorbed, bool crit, AttackConditions attackConditions)
        {
            ...
            // ── Mana Barrier ───────────────────────────────────────────────────────
            var mbResult = TryAbsorbWithManaBarrier(ref amount, damageType);
