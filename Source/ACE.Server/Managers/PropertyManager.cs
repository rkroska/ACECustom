...
namespace ACE.Server.Managers
{
    ...
    public static class ServerConfig
    {
        ...
        public static ConfigProperty<bool> encounter_spawn_base_variation_only { get; private set; } = new(false, "if TRUE, world encounter table spawns are skipped only on prestige landblock variations (variation > 10). Retail variations 1-10 and the base landblock (NULL; 0 is always base) still run encounters. FALSE = same encounter rows for all variations (stock ACE).");
        public static ConfigProperty<bool> encounter_spawn_base_layer_only { get; private set; } = new(true, "if TRUE, world encounter table spawns (outdoor random generators from the encounter DB) run only on the base landblock: VariationId NULL (0 is always base and is converted to NULL on load). Skips retail layers 1-10 and prestige 11+. FALSE = stock ACE (encounters on every variation unless encounter_spawn_base_variation_only blocks prestige).");
