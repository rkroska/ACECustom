...
namespace ACE.Server.Managers
{
    ...
    public static class ServerConfig
    {
        ...
        public static ConfigProperty<bool> guid_reuse_advance_instance { get; private set; } = new(true, "Invisible-mob fix (2026-09-18). If TRUE, every object on a dynamic GUID (new, or restored from the database) starts its ObjectInstance sequence at a server-uptime value (60-minute units), so a client still holding a record for an earlier owner of a recycled GUID replaces it instead of treating the new object as the old one (a creature that is never drawn but still attacks). FALSE = stock ACE (constant instance 0). READ ONCE AT STARTUP: a change applies on the next restart only, because the client rejects an instance lower than one it already saw on that GUID, so switching off mid-session would hide new objects. Kill switch for the first days on live, to be removed afterwards. To disable: /modifybool guid_reuse_advance_instance false, then restart.");
        ...
        public static ConfigProperty<bool> encounter_spawn_base_variation_only { get; private set; } = new(false, "if TRUE, world encounter table spawns are skipped only on Prestige landblock variations (PrestigeManager.IsPrestigeVariation: variation > 1000). The base landblock (NULL; 0 is always base) and every layer from 1 to 1000 still run encounters. FALSE = same encounter rows for all variations (stock ACE).");
        public static ConfigProperty<bool> encounter_spawn_base_layer_only { get; private set; } = new(true, "if TRUE, world encounter table spawns (outdoor random generators from the encounter DB) run only on the base landblock: VariationId NULL (0 is always base and is converted to NULL on load). Skips every explicit layer (any non-NULL variation). FALSE = stock ACE (encounters on every variation unless encounter_spawn_base_variation_only blocks prestige).");
        ...
        public static ConfigProperty<bool> room_assign_test_tools { get; private set; } = new(false, "Dungeon TEST tools (/zonecontrol dungeon fill, clear, admin, markers, doors show): fake players that make rooms count as taken, admins counting as players, landing markers, a door preview. They change real hand-out decisions or spawn objects, so they are OFF unless this is true. Change live: /modifybool room_assign_test_tools true");
        ...
        public static ConfigProperty<long> room_assign_leave_hold_seconds { get; private set; } = new(120, "Room Assign: seconds a one-player room stays held for the ACCOUNT of a player who teleported out of it (recall, portal, death), so they return to the same room. 120 s (owner 2026-09-23): long enough to nip out of the dungeon for a fast trade and come back to the same chamber. 0 = no leave holds, max 86400. Change live: /modifylong room_assign_leave_hold_seconds <seconds>");
        public static ConfigProperty<long> room_assign_logout_hold_renewals { get; private set; } = new(1, "Room Assign: extra logout holds an ACCOUNT may take after the first, in a dungeon before it must be handed a room there that is new to it (relogging into its own room does not refill them; ending the visit - logging out outside every room - does). Stops relogging to keep a room forever; 1 still covers crashing twice. 0-100. Change live: /modifylong room_assign_logout_hold_renewals <count>");
        ...
        public static ConfigProperty<long> pet_breeding_allowed_landblock { get; private set; } = new(0x0106L, "Where breeding is allowed. A 16-bit landblock (0x0106 = Ruggan's Annex, the Seedy Motel) matches the whole landblock; a full 32-bit cell (0x01060186) matches only that exact cell. 0 = allowed anywhere.");
        public static ConfigProperty<long> pet_breeding_allowed_variant { get; private set; } = new(2, "Landblock variation ID where breeding is allowed (2 = Ruggan's Annex; the base layer of 0x0106 is a retail dungeon). -1 = ignore variation.");
        ...
        public static ConfigProperty<long> pet_breeding_vitality_mutation_step { get; private set; } = new(50, "Max health bonus granted per vitality mutation.");
        ...
        public static ConfigProperty<long> pet_breeding_potency_hard_cap { get; private set; } = new(0, "Maximum allowable potency from breeding (0 = uncapped). The effective cap is the smallest positive of this and pet_potency_max_stored.");
        ...
        public static ConfigProperty<double> pet_breeding_guardian_damage_mult { get; private set; } = new(0.5, "True multiplier on the mating guardian's outgoing base damage (0.5 = half). The base hit is 8% of the defending parent pet's max health clamped to 20-500, then this multiplier, then halved again if an Offering of Subjugation was consumed.");
        ...
        public static ConfigProperty<bool> pet_sex_icon_underlay_enabled { get; private set; } = new(true, "If TRUE, every combat pet essence shows a sex-coloured square behind its inventory icon, so male and female can be told apart without appraising.");
        public static ConfigProperty<long> pet_sex_icon_underlay_male { get; private set; } = new(100670255, "Icon underlay DataId used for male essences (0x06001B2F). 0 = leave the icon alone.");
        public static ConfigProperty<long> pet_sex_icon_underlay_female { get; private set; } = new(100670253, "Icon underlay DataId used for female essences (0x06001B2D). 0 = leave the icon alone.");
        ...
        public static ConfigProperty<bool> pet_trace { get; private set; } = new(false, "If TRUE, writes the full copy-paste [PetTrace] session trace at INFO: every breeding gate, config value, inheritance line, roll, REPLAY blob, guardian event, birth, maturity credit, consumable, tailoring and neutering use, AND every combat exchange server-wide (every melee/missile/magic hit, evade, DoT tick, heal and death, for players, monsters, pets and guardians) with the full damage arithmetic. EXTREMELY verbose: for a controlled test session only, never on a live shard. Off = zero cost.");
        ...
        public static ConfigProperty<long> content_template_export_wcid_start { get; private set; } = new(78790000, "@export-template / @et / @ed template: first wcid of the TEMPORARY export staging block. Automatic ids come only from [start, end] (the 7879 prefix; 7878 stays hand-authored), and @id / @import-sql / @import-json refuse a wcid inside it unless the word force is added. See docs/WCID_ALLOCATION_7878.md.");
        public static ConfigProperty<long> content_template_export_wcid_end { get; private set; } = new(78799999, "@export-template: last wcid of the export staging block (inclusive). Raise it when the block is used up; the tool never wraps around.");
        public static ConfigProperty<bool> content_template_export_auto_import { get; private set; } = new(true, "@export-template / @et: load the exported SQL straight into ace_world and drop the weenie from the cache, so it can be spawned at once with @ci / @create. Only exports inside the staging block are loaded; an explicit id outside it is never auto-loaded, so an overwrite cannot silently replace real content.");
        public static ConfigProperty<bool> content_template_export_auto_discord { get; private set; } = new(true, "@export-template / @et: also send the exported file to the Discord exports channel, as @ed template does. Skipped when no exports channel is configured.");
        public static ConfigProperty<long> content_template_export_next_wcid { get; private set; } = new(0, "@export-template: persisted high-water mark, the next wcid an automatic export may hand out. Advanced and written to the shard database BEFORE each file is produced, so two exports made before either SQL file is loaded (the database still shows both ids free) can never receive the same id. Reset to 0 (or the block start) only when moving the block.");
